    ...
    <main style={{ maxWidth: 560, margin: '0 auto', padding: 20 }}>
        ...
        <div style={{ border: '1px solid #f0a', borderRadius: 8, padding: 16, marginBottom: 24 }}>
          ...
          <details style={{ marginBottom: 12 }}>
            ...
            <form action={modifierDefiMensuel} style={{ display: 'flex', flexDirection: 'column', gap: 8, maxWidth: 460, marginTop: 10 }}>
              ...
              <label style={{ fontSize: 12, opacity: 0.7 }}><EmojiBeast /> Mode Beast (dépasser l'or) — optionnel, laisser vide pour ne pas proposer ce niveau</label>
          ...
          {defiActuel.description_beast && (
            <p style={{ fontSize: 12, margin: '4px 0' }}><strong><EmojiBeast /> Beast :</strong> {defiActuel.description_beast}</p>
          )}
          ...
          {enAttente.map((p) => (
            <form
              action={validerParticipationDefi}
              key={p.id}
              style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderBottom: '1px solid #333' }}
            >
          ...
          {validees.map((p) => (
            <div key={p.id} style={{ padding: '6px 0', borderBottom: '1px solid #333' }}>
              <form action={surclasserNiveauParticipation} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <button type="submit" style={{ fontSize: 12, padding: '5px 12px', borderRadius: 999, border: '1px solid #555', background: 'none', color: '#ccc', cursor: 'pointer' }}>
                  Modifier
                </button>
                <button
                  type="submit"
                  formAction={invaliderParticipationDefi}
                  style={{ fontSize: 11, padding: '5px 10px', borderRadius: 999, border: '1px solid #666', background: 'none', color: '#999', cursor: 'pointer' }}
                >
                  Annuler
                </button>
              </form>
              {p.tentative_superieure && (
                <div style={{ marginTop: 6, fontSize: 12, color: '#fc6', display: 'flex', alignItems: 'center', gap: 8 }}>
                  🎯 Tente le niveau {LABEL_NIVEAU[p.tentative_superieure]} — vidéo à vérifier
                  <form action={surclasserNiveauParticipation}>
                    <input type="hidden" name="participation_id" value={p.id} />
                    <input type="hidden" name="niveau" value={p.tentative_superieure} />
                    <button type="submit" style={{ fontSize: 11, padding: '3px 10px', borderRadius: 999, border: '1px solid #4a4', background: 'none', color: '#8f8', cursor: 'pointer' }}>
                      ✓ Confirmer le surclassement
                    </button>
                  </form>
                </div>
              )}
      ...
      <section style={{ marginBottom: 32 }}>
        ...
        <form action={creerDefiMensuel} style={{ display: 'flex', flexDirection: 'column', gap: 8, maxWidth: 460 }}>
          ...
          <label style={{ fontSize: 12, opacity: 0.7 }}><EmojiBeast /> Mode Beast (dépasser l'or) — optionnel</label>
      ...
      <section>
        ...
        {(defis ?? []).map((d, i) => (
          <details key={d.id} style={{ borderBottom: '1px solid #333', padding: '10px 0' }}>
            ...
            {d.description_beast && <p style={{ fontSize: 12, margin: '4px 0 8px' }}><EmojiBeast /> {d.description_beast}</p>}
            {i !== 0 && (
              <form action={supprimerDefiMensuel}>
